> # ⚠️ SUPERSEDED — RETAINED AS A RECORD. DO NOT FOLLOW THIS DOCUMENT.
>
> This design document specified `simulate_training_curves()`, a function that
> generated a **synthetic** reward curve (a sigmoid plus Gaussian noise) and a
> **synthetic** loss curve (an exponential decay plus noise). It then instructed
> that those plots be committed to the repository and embedded inline in the
> README as evidence of training. **No model was ever trained. No checkpoint
> exists. Those curves depicted a training run that never happened.**
>
> The results tables in the **FILE 8** (`app.py`) and **FILE 10** (`README.md`)
> templates below are **invented, not measured**. They were transcribed verbatim
> into the published README and shipped for months.
>
> Real measurement reverses their ordering. This document predicted a trained LLM
> at 68% / 61% / 22.7 beating a heuristic at 26% / 18% / 4.1. Measured over 100
> seeded episodes, the heuristic scores **100% / 100% / 52.98** — above every LLM
> figure this document predicted. The spec asserted the narrative it wanted; the
> implementation published the assertion.
>
> **The only valid source of numbers for this project is
> [`eval/results.json`](eval/results.json)**, written by `eval/evaluate.py` from
> a seeded, reproducible run and carrying a content fingerprint of the code it
> measured.
>
> The instructions below are left **visible and unedited**, each annotated where
> it was not followed, so the record of what was specified stays legible. Nothing
> here should be treated as guidance.
...
> ⚠️ **NOT FOLLOWED.** The PNG was synthetic and has been deleted.
...
> ⚠️ **NOT FOLLOWED.** The plots were synthetic; the README no longer embeds them.
...
├── eval/
│   ├── __init__.py
│   └── evaluate.py           ← runs all 3 agents, saves curves  # ⚠️ NOT FOLLOWED: only 2 agents were ever evaluated; the LLM row was never measured
├── training_curves/
...
> ⚠️ **NOT FOLLOWED.** The curve-generating code below was removed from the implementation. The curves this refers to were synthetic — generated by `simulate_training_curves()`, never produced by a training run. The PNGs have been deleted and this instruction was not carried out. See `eval/results.json` for the only measured numbers.
...
def simulate_training_curves(n_episodes=500):
    # ⚠️ BODY REMOVED. It fabricated a reward curve (sigmoid + Gaussian noise)
    # and a loss curve (exponential decay + noise), which were then plotted,
    # committed, and embedded in the README as evidence of GRPO training that
    # never happened. Deleted so it cannot be copied out. See eval/results.json.
    raise NotImplementedError("removed: generated synthetic training curves")
...
if __name__ == "__main__":
    ...
    run_leaderboard()
    print("\nDone. Commit training_curves/*.png to the repo.")  # ⚠️ NOT FOLLOWED
```
...
# ⚠️ INVENTED, NOT MEASURED. These numbers originate here and were copied
# verbatim into app.py and README.md at the first commit. Measured values:
# Random 16%/4%/-18.22, Heuristic 100%/100%/52.98 (n=100, seed=12345).
# The 'Trained LLM' row was never measurable: no trainer, no checkpoint,
# and no LLM was ever present in eval/evaluate.py. See eval/results.json.
"""
...
> ⚠️ **NOT FOLLOWED.** The plots were synthetic and are no longer embedded. The curves this refers to were synthetic — generated by `simulate_training_curves()`, never produced by a training run. The PNGs have been deleted and this instruction was not carried out. See `eval/results.json` for the only measured numbers.
...
> ⚠️ **NOT FOLLOWED.** This entire section was removed from the published README. The curves this refers to were synthetic — generated by `simulate_training_curves()`, never produced by a training run. The PNGs have been deleted and this instruction was not carried out. See `eval/results.json` for the only measured numbers.
...
> ⚠️ **NOT FOLLOWED.** These figures are invented. The published table now reads Random 16% / 4% / -18.22 and Heuristic 100% / 100% / 52.98, measured over n=100 seeded episodes; the LLM row is gone. See `eval/results.json`.
...
python eval/evaluate.py   # generates training_curves/*.png   # ⚠️ NOT FOLLOWED: now writes eval/results.json; generates no curves
...
Cell 7: Plot reward + loss curves, save as PNG   # ⚠️ NOT FOLLOWED — removed; it regenerated the synthetic curves
...
7. COMMIT training_curves/*.png to repo NOW
   # ⚠️ NOT FOLLOWED — steps 6-7 produced and committed synthetic curves.
   # The PNGs are deleted; evaluate.py now writes eval/results.json instead.
